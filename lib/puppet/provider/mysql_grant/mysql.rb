...
Puppet::Type.type(:mysql_grant).provide(:mysql, :parent => Puppet::Provider::Mysql) do
  ...
  def self.instances
    ...
    users.select{ |user| user =~ /.+@/ }.collect do |user|
      ...
      grants.each_line do |grant|
        ...
        # Matching: GRANT (SELECT, UPDATE) PRIVILEGES ON (TABLE, FUNCTION, PROCEDURE) (*.*) TO ('root')@('127.0.0.1') (WITH GRANT OPTION)
        if match = munged_grant.match(/^GRANT\s(.+)\sON\s(.+)\s(.+)\sTO\s(.*)@(.*?)(\s.*)$/)
          privileges, object_type, table, user, host, rest = match.captures
  ...
  def grant(user, object_type, table, privileges, options)
    user_string = self.class.cmd_user(user)
    object_string = self.class.cmd_object_type(object_type)
    priv_string = self.class.cmd_privs(privileges)
    table_string = self.class.cmd_table(table)
    query = "GRANT #{priv_string} ON"
  ...
  def create
    grant(@resource[:user], @resource[:object_type], @resource[:table], @resource[:privileges], @resource[:options])

    @property_hash[:ensure]      = :present
    @property_hash[:object_type] = @resource[:object_type]
  ...
  def revoke(user, object_type, table)
    user_string        = self.class.cmd_user(user)
    object_type_string = self.class.cmd_object_type(object_type)
    table_string       = self.class.cmd_table(table)

    query = "REVOKE ALL ON #{object_type} #{table_string} FROM #{user_string}"
    mysql([defaults_file, '-e', query].compact)
  end

  def destroy
    revoke(@property_hash[:user], @property_hash[:object_type], @property_hash[:table])
    @property_hash.clear
  ...
  def privileges=(privileges)
    revoke(@property_hash[:user], @property_hash[:object_type], @property_hash[:table])
    grant(@property_hash[:user], @property_hash[:object_type], @property_hash[:table], privileges, @property_hash[:options])
  ...
  def options=(options)
    revoke(@property_hash[:user], @property_hash[:object_type], @property_hash[:table])
    grant(@property_hash[:user], @property_hash[:object_type], @property_hash[:table], @property_hash[:privileges], options)
